...
- A system administrator will modify some HTML/CSS files to create a company header/footer for document PDFs
- A system administrator will make use of Django's admin pages to enter and maintain some supporting "system data" (i.e. an address book; prices, including special resale deals; products, including details about modular products and standard accessories)
...
There should be one Product record for each item on sale and each standard accessory.

Descriptions of Products are stored as separate records, allowing users to add multilingual support and to modify the descriptions over time (while storing the old descriptions for reference).

Slot and SlotChoiceList are used to describe "modular" Products. Suppose the company sells a "joyfully toy-full chocolate egg" which contains 1-4 small toys of the customer's choice: this would be considered a modular product because the egg alone is incomplete, the customer must also order 1-4 toys (for this example, let's say 0 toys is disallowed for business reasons and 5 wouldn't fit). SlotChoiceList is where Adminas gets the list of suitable small toys: a name is assigned for the convenience of system admins (e.g. "toys <6 cm") then it has an MTM table for the toy Products. Slot is where Adminas links the list of toys to the egg and describes how many are needed/allowed. It has a FK to the joyfully toy-full chocolate egg Product; a FK to the "toys <6 cm" SlotChoiceList; a name for display on the module management page (e.g. "Toy Inside"); that 1 is required and an additional 3 are optional. Multiple Slots can be assigned to a single Product in the event of a more complex modular item. A single SlotChoiceList can be used to populate multiple different Slots in multiple different Products (it's up to the system administrator to judge if that's a good idea).
...
PriceList sets the "valid from" date for a set of prices and assigns a name to the price list (e.g. "2022 Q1") which will appear on documents and is intended to be the way the company and their customers/agents refer to the price list. Price stores one item on the price list: a currency, a value, and FKs to the Product and the PriceList. Together these set the "normal" prices.

The two "Resale" models are for situations where the company sells both directly to customers and via resellers/agents. Chances are there'll be some sort of resale discount on offer for the resellers/agents, but not necessarily the same resale discount on all Products and also not necessarily the same resale discount to all agents.

ResaleCategory is used for "default" resale discounts, based solely on the Product: it has fields for a category name and the percentage. Adminas assumes that each Product will only have one default resale discount (if a Product is in two categories where one has 10% resale and one has 20% resale, all agents will insist on applying the 20% category every time, so its presence in the 10% category would only serve to waste time in fruitless discussions). Since this makes it a OTM field, the Product model contains a "resale_category" FK to the ResaleCategory.
...
Sites can also be accessed separately, with Addresses included in tabular format. When updating an address, odds are that the system administrator will be thinking in terms of "Acme's accounting office has moved" rather than street addresses, so this is intended to make it easier to find the correct address. It also serves to show an address history for each Site, should that be of interest.

AgentResaleGroups can also be accessed separately, with a list of included Products in tabular format. This allows modifications to the list of Products included in each AgentResaleGroup (and the percentages, if the user is so inclined).

Products has Descriptions, StandardAccessories and Slots displayed tabularly. Descriptions are a key piece of Product information and it makes sense to access them via the Product, since users are likely to think in terms of "I need to adjust the EN description of ProductX". The presence of StandardAccessories and Slots means users can see a full list of all Products supplied with this Product and the details of any "empty slots" for adding more Products, all in one space. As with Descriptions, users are likely to think of StandardAccessories and Slots in terms of the Product to which they belong. All three of these models can only be accessed via Products in Django admin.

SlotChoiceLists only contains itself, allowing users to modify the names and which Products are valid options for each choice list. 
...
JobItems are the means by which Products are added to Jobs, with the ability to also state the selling price, the applicable price list, and how many are required.


## Modular JobItems Model
The JobItem-specific part of this is handled by just one model, JobModule. Via FK fields it links the parent JobItem, the child JobItem and the Slot being filled, to which it adds a quantity (necessary because JobItem has a quantity field and users won't necessarily want all of them to fill the same slot).
...
WOs are internal documents, so it's unnecessary to consider the requirements of customers or third-parties. Consistent wording and display on WOs reduces the risk of miscommunication and increases the efficiency of the recipient by allowing them to recognise products at a glance, so it's probably in the interests of the user's company. When a modified version of a product is being supplied, there's a risk of the WO recipient overlooking the modifications and supplying the standard version, so it's important to make the modifications very eye-catching and obvious: a tiny edit in the middle of an otherwise familiar sentence would be insufficient. Adminas instead allows users to add SpecialInstructions to their WOs (which are displayed in a big obvious box at the top).

One of the purposes of an OC is to translate the customer's PO into terms understood by the company, then advise the customer of this understanding so they can confirm it's correct. Due to this, rewording the OC to exactly match the wording and layout of the customer's PO rather defeats the purpose. Since OCs are rarely sent to third-parties, third-party confusion isn't a factor. Customers might not even be aware that their modifications are modifications, so the argument for preventing description editing is less clear in this case than on the WO. Still, modifications don't come up all that often and SpecialInstructions can do the job of communicating 2message received" to the customer, so Adminas reuses that approach.
...
A large percentage of paper documents stored in filing cabinets are never again seen by human eyes. Assuming the same is true of digital documents, I decided against storing the PDFs: it would use more memory compared to storing just the strings and while it'd reduce the amount of database access (one filename vs. multiple tables and strings), that only happens when you issue the document and when you look at it.

I decided against locking records for editing because it'd be annoying to keep having to remake records and set up all the FKs links every time a user finds a typo in something that's gone out.
